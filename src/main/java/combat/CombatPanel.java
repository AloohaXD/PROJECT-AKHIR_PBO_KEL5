...
public class CombatPanel extends JPanel {
    ...
    public int currentChapter = 1;  // set by GamePanel before startCombat
    private Hero  hero;
    private Enemy enemy;
    ...
    // === CHARACTER SWITCH ===
    private HeroClass.CharacterProfile[] partyProfiles = null; // all 5 profiles
    private int activePartyIdx = 0;  // which hero is currently fighting
    private JPanel switchPanel = null;
    private JButton[] switchBtns = null;
    ...
    public void startCombat(HeroClass.CharacterProfile profile) {
        startCombat(profile, null);
    }
    public void startCombat(HeroClass.CharacterProfile profile, Enemy customEnemy) {
        startCombat(new HeroClass.CharacterProfile[]{profile}, 0, customEnemy);
    }
    /** Full party start — BUG FIX: pass all party profiles for character switching */
    public void startCombat(HeroClass.CharacterProfile[] profiles, int startIdx, Enemy customEnemy) {
        this.partyProfiles = profiles;
        this.activePartyIdx = startIdx;
        this.hero          = new Hero(profiles[startIdx]);
        this.enemy         = (customEnemy != null) ? customEnemy : Enemy.createOrc();
        this.gameOver      = false;
        ...
        heroSpritePanel.setClassType(profiles[startIdx].classType);
    ...
    /** Switch to a different party member mid-combat */
    private void switchCharacter(int idx) {
        if (partyProfiles==null||idx==activePartyIdx||idx>=partyProfiles.length) return;
        if (!playerTurn || gameOver) return;
        activePartyIdx = idx;
        hero = new Hero(partyProfiles[idx]);
        heroSpritePanel.setClassType(partyProfiles[idx].classType);
        logMsg("--- Berganti ke " + partyProfiles[idx].name + "! ---", BLUE);
        updateAllUI();
        updateSwitchPanel();
    ...
    private void updateSwitchPanel() {
        if (switchBtns==null||partyProfiles==null) return;
        for (int i=0;i<switchBtns.length;i++) {
            if (i>=partyProfiles.length) { switchBtns[i].setVisible(false); continue; }
            switchBtns[i].setVisible(true);
            HeroClass.ClassType cls=partyProfiles[i].classType;
            boolean active=(i==activePartyIdx);
            switchBtns[i].setText("<html><center><font color='" + (active?"#FFD700":"#AAAADD") + "'>"
                +cls.displayName+"<br><font size='-2'>HP:"+partyProfiles[i].maxHp+"</font></font></center></html>");
            switchBtns[i].setBorder(active
                ? javax.swing.BorderFactory.createLineBorder(new Color(255,200,50),2)
                : javax.swing.BorderFactory.createLineBorder(new Color(60,60,80),1));
        }
    }
    ...
    private JPanel buildHandSection() {
        ...
        switchWrapper.setBackground(BG_DARK);
        JLabel switchLabel = new JLabel("  Switch Hero:");
        switchLabel.setFont(new Font("Serif",Font.BOLD,11));
        switchLabel.setForeground(new Color(160,150,200));
        switchWrapper.add(switchLabel,BorderLayout.WEST);
        switchPanel = new JPanel(new GridLayout(1,5,4,0));
        switchPanel.setBackground(BG_DARK);
        switchBtns = new JButton[5];
        for(int i=0;i<5;i++){
            final int idx=i;
            switchBtns[i]=new JButton("?");
            switchBtns[i].setFont(new Font("Monospaced",Font.PLAIN,10));
            switchBtns[i].setForeground(TEXT_MAIN);
            switchBtns[i].setBackground(new Color(25,22,50));
            switchBtns[i].setFocusPainted(false);
            switchBtns[i].setBorderPainted(true);
            switchBtns[i].setPreferredSize(new Dimension(0,40));
            switchBtns[i].addActionListener(e->switchCharacter(idx));
            switchPanel.add(switchBtns[i]);
        }
        switchWrapper.add(switchPanel,BorderLayout.CENTER);
        wrapper.add(switchWrapper,BorderLayout.SOUTH);
        return wrapper;
    }
    ...
    class SpritePanel extends JPanel {
        ...
        @Override protected void paintComponent(Graphics g) {
            ...
            // FIX: use ImageCache (no per-frame disk I/O)
            String bgPath="/assets/images/combat_bg/ch"+CombatPanel.this.currentChapter+".jpg";
            java.awt.image.BufferedImage bgImg=main.ImageCache.get().get(bgPath);
            if(bgImg!=null){
                g.drawImage(bgImg,0,0,getWidth(),getHeight(),null);
                g.setColor(new Color(0,0,0,110));
                ((Graphics2D)g).fillRect(0,0,getWidth(),getHeight());
            } else {
                g.setColor(BG_DARK); ((Graphics2D)g).fillRect(0,0,getWidth(),getHeight());
            }
        ...
        private void drawHero(Graphics2D g2, int W, int H) {
            ...
            if (img!=null) drawFit(g2,img,W,H);
                ...
                // FIX: use ImageCache for hero sprite
                String sp=(classType!=null)?classType.spritePath:"/assets/images/karakter/warrior.png";
                java.awt.image.BufferedImage si=main.ImageCache.get().get(sp);
                if(si!=null) main.ImageCache.drawContain(g2,si,(W-90)/2,5,90,H-20);
            }
            // label
            if (classType!=null) {
        ...
        private void drawEnemy(Graphics2D g2, int W, int H) {
            ...
            if (img!=null) drawFit(g2,img,W,H);
                ...
                // FIX: use ImageCache for enemy sprite — use boss or kroco image based on name
                int ch2=CombatPanel.this.currentChapter;
                String ep="/assets/images/boss/boss_ch"+ch2+".png";
                java.awt.image.BufferedImage ei=main.ImageCache.get().get(ep);
                if(ei!=null) main.ImageCache.drawContain(g2,ei,(W-110)/2,5,110,H-20);
            }
            // label
            g2.setFont(new Font("Serif",Font.BOLD,11));
